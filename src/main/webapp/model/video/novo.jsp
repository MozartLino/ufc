	...
	<div class="main">
		...
		<div class="container_24 relative">
			<article class="grid_14 a2">
				<div class="inner">
					<h3 class="hp-1">Cadastro Video</h3>
					<div class="hr"></div>
					<form id="contact-form" action="salva" method="POST">
						<!--<div class="success" style="display: none; "> Contact form submitted! <strong>We will be in touch soon.</strong>
                      </div>-->
						<fieldset>
							<p>
								<label> 1° Lutador:<br /> <select name="video.lutadorId1" class="estado select" id="lutador1">
							...
							<p>
								<label> 2° Lutador:<br /> <select name="video.lutadorId2" class="estado select" id="lutador2">
										<option value="sec">Selecionar...</option>
										<c:forEach var="lutador" items="${lutadorList}">
											<option value="${lutador.id}">${lutador.nome}</option>
										</c:forEach>
							...
							<p>
								<label class="name">Url do video:<br /> <input
									type="text" value="" name="video.url"/>
								</label>
							</p>
							<div class="clear"></div>
							<div class="btns">
